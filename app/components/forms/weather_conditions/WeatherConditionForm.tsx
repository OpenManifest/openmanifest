...
export default function WeatherConditionForm(props: IWeatherConditionFormProps) {
  ...
  const [temperature, setTemperature] = React.useState<number | null | undefined>(state.fields?.temperature?.value || 0);
            ...
            return (
              <WindRow
                ...
                onChange={(field, value) => {
                  const newWinds = set([...(winds as Wind[])], index, { ...wind, [field]: value });
                  dispatch(
                    actions.forms.weather.setField([
                      "winds",
                      newWinds
