...
export default function JumpHistoryTab(props: IJumpHistoryTab) {
  ...
  return (
    <View animation={currentTabIndex > tabIndex ? "slideInLeft" : "slideInRight"} easing="ease-in-out" duration={200} style={{ padding: 8 }}>
        ...
        {data.map((item) =>
          !item?.node ? null :
            <SlotCard
              slot={item.node}
              onPress={() => {
                item?.node?.load?.id && navigation.navigate('Authenticated', {
                  screen: 'Drawer',
                  params: {
                    screen: 'Manifest',
                    params: {
