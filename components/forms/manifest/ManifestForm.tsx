  ...
  React.useEffect(() => {
    if (state.fields.user?.value) {
      if (!state.fields.exitWeight.value) {
        dispatch(
          actions.forms.manifest.setField(["exitWeight", Number(state.fields.user.value.exitWeight || 60)])
        );
      }

      if (!state.fields.rig.value && state.fields.user.value.rigs?.length) {
        dispatch(
          actions.forms.manifest.setField(["rig", state.fields.user.value.rigs[0]])
        );
      }
